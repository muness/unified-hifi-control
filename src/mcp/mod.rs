...
//!
//! Provides an HTTP/SSE endpoint at `/mcp` for mobile MCP clients like BoltAI iOS.
...
impl HifiMcpServer {
    ...
    async fn hifi_browse(
        ...
        match self.state.roon_browse.browse(opts).await {
            Ok(result) => {
    ...
    async fn hifi_browse_status(&self) -> Result<CallToolResult, McpError> {
        let connected = self.state.roon_browse.is_connected().await;
